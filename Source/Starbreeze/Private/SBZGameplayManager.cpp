#include "SBZGameplayManager.h"

USBZGameplayManager::USBZGameplayManager() {
}

void USBZGameplayManager::OnRestartLevelStarted() {
}

void USBZGameplayManager::OnPlayableLevelInitialized(UWorld* World) {
}

void USBZGameplayManager::OnOutroSequenceStarted(const int32 OutroVariation) {
}

void USBZGameplayManager::OnBlackScreenStarted() {
}

void USBZGameplayManager::OnActionPhaseStarted() {
}

void USBZGameplayManager::OnActionPhaseExited() {
}

USBZGameplayManager* USBZGameplayManager::Get(const UObject* WorldContextObject) {
    return NULL;
}


